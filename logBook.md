# Logbook

| Date  | Used hours | Subject(s) |  Outcome(s) |
| :---         |     :---:      |     :---:      |     :---:      |
| 02.11.2025 | 4 | Cisco task | Have done a couple of exams |
| 03.11.2025 | 2 | Cisco task | Finished all of exams |
| 03.11.2025 | 1 | PortSwigger | Explored the PortSwigger environment |
| 04.11.2025 | 3 | PortSwigger | Completed 6 labs |
| 14.11.2025 | 2 | The Booking system project → Phase 1 → Part 1 | read the instructions an dhave done pasrt of the task |
| 16.11.2025 | 3 | The Booking system project → Phase 1 → Part 1 | Watch the tutorial video + Complete the assignment |
| 24.11.2025 | 2.75 | The Booking system project → Phase 1 → Part 1 | Fix ZAP report |
| 25.11.2025 | 3 | The Booking system project → Phase 1 → Part 2 | report to development team |
| 04.12.2025 | 4.5 | The Booking system project → Phase 2 | assignment(cracking passwords) |
| 10.12.2025 | 4 | The Booking system project → Phase 3 | assignment(collect all the data) |
| 15.12.2025 | 2.5 | The Booking system project → Phase 3 | assignment(finish checking the app, submit the task) |
| 15.12.2025 | 3 | The Booking system project → Phase 4 | assignment |
| 17.12.2025 | 4 | The Booking system project → Phase 4 | watch video + submit the task |
| 17.12.2025 | 2.5 | portswigger labs | portswigger labs   |
| 17.12.2025 | 2 | Final assignment | do the task   |

